...
这是因为我们在懒加载的时候，需要确定某个特定的文件偏移在外存中对应的簇号，也就是映射：
$$
offset \rightarrow cluster
$$
因为 FAT32 是链表，在检索的时候是顺序访问，所以时间复杂度是 $O(offset/512)$ ，这样的效率是很低的，我们需要随机访问特性结构。比较容易想到的是数组描述，但是因为可执行文件的大小不同，最大的可执行文件需要的数组过大，所以定长数组浪费内存。
...
在 `dirmeta` 结构体中，记录该文件访问过的最大簇号。如果需要读写的簇低于这个最大簇，就能实现 O(1) 查找。如果超过了这个簇，再通过 FAT 表查询，并一边查询一边更新这个 `Inode`。
