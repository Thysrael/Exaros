...
| -------- | ------------------------------------------------------------ | ------ |
| brk      | 更改数据段的尾部地址                                         | 已实现 |
| madvise  | 通知内核对指定的内存区域采取特定的操作，例如，预读、预写、释放等 |        |
| mlock    | 将指定的内存区域锁定在物理内存中，以便防止被交换出去。       |        |
| mmap     | 将一个文件或设备映射到进程的地址空间中                       |        |
| mprotect | 修改指定内存区域的访问权限                                   |        |
| mremap   | 重新映射一个已经映射的内存区域                               |        |
| msync    | 将内存中的数据同步到磁盘上                                   |        |
| munlock  | 解锁指定的内存区域                                           |        |
| munmap   | 解除一个进程地址空间中的内存映射                             |        |
...
| linkat      | 创建软链接（似乎在 linux 上是硬链接）    |        |
| lremoveattr | 删除指定文件的扩展属性                   |        |
| lseek       | 设置文件的偏移量                         |        |
| lsetxattr   | 设置指定文件的扩展属性                   |        |
| mkdirat     | 创建目录                                 | 已实现 |
| mount       | 挂载文件系统                             | 已实现 |
| newfstatat  | 获得文件状态（功能强大）                 |        |
| openat      | 打开文件                                 | 已实现 |
| pipe2       | 创建一个管道                             | 已实现 |
| read        | 读取文件                                 | 已实现 |
| readahead   | 预读取文件的内容到内存中                 |        |
| readlinkat  | 读取符号链接文件的目标路径               |        |
| readv       | 从文件中读取多个数据块到多个缓冲区中     |        |
